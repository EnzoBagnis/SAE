...
(function () {
    ...
    const RID  = window.RESOURCE_ID || null;
    ...
    function liItem(text, onClick) {
        ...
        li.style.cssText = 'padding:9px 10px;border-bottom:1px solid #f0f0f0;font-size:.9em;cursor:' + (onClick ? 'pointer' : 'default');
        li.textContent = text;
        if (onClick) {
    ...
    // ── DOM ──────────────────────────────────────────────────────────────────
    const inputEl  = document.getElementById('resourceSearchInput');
    const typeEl   = document.getElementById('resourceSearchType');
    const clearBtn = document.getElementById('resourceClearBtn');
    const resDiv   = document.getElementById('resourceSearchResults');
    const exDiv    = document.getElementById('rsr-exercises');
    const exList   = document.getElementById('rsr-exercises-list');
    const stDiv    = document.getElementById('rsr-students');
    const stList   = document.getElementById('rsr-students-list');
    const modal    = document.getElementById('rsrStudentModal');
    const mTitle   = document.getElementById('rsrStudentModalTitle');
    const mBody    = document.getElementById('rsrStudentModalBody');

    if (!inputEl) return; // sécurité si la barre n'est pas rendue
    ...
    async function doSearch() {
        ...
        if (type === 'exercises') {
            ...
            try {
                let url = `${BASE}/api/dashboard/exercises`;
                if (RID) url += `?resource_id=${RID}`;
                ...
                if (!matches.length) {
                    ...
                    matches.forEach(e => {
                        ...
                        exList.appendChild(liItem(name, () => {
            ...
            try {
                let url = `${BASE}/api/dashboard/students?page=1&perPage=100000`;
                if (RID) url += `&resource_id=${RID}`;
                const resp = await fetch(url);
                if (!resp.ok) throw new Error('API error');
                const json = await resp.json();
                ...
                if (!matches.length) {
                    ...
                    matches.forEach(s => {
                        ...
                        stList.appendChild(liItem(label, () => openStudentDetail(sid)));
                    });
                }
            } catch (err) {
    ...
    async function openStudentDetail(studentId) {
        ...
        mBody.innerHTML        = '<p style="color:#888;">Chargement…</p>';

        try {
            let url = `${BASE}/api/dashboard/student/${encodeURIComponent(studentId)}`;
            if (RID) url += `?resource_id=${RID}`;
            const resp = await fetch(url);
            if (!resp.ok) throw new Error('API error');
            const json = await resp.json();
            ...
            if (!json.success) {
                mBody.innerHTML = '<p style="color:#888;">Aucune donnée disponible.</p>';
                return;
            }

            const attempts = json.data.attempts || [];
            if (!attempts.length) {
                mBody.innerHTML = '<p style="color:#666;">Aucune tentative enregistrée pour cet étudiant dans cette ressource.</p>';
                return;
            }
            ...
            attempts.forEach(a => {
                const ok = a.correct ? '✅' : '❌';
                html += `<li style="padding:9px 0;border-bottom:1px solid #f0f0f0;">
            ...
            console.error(err);
            mBody.innerHTML = '<p style="color:#e74c3c;">Erreur lors du chargement.</p>';
        }
    }
