...
// Vérifier les permissions d'accès à la ressource (propriétaire ou observateur)
$hasAccess = false;
if ($resource->owner_user_id === $user_id) {
    $hasAccess = true; // L'utilisateur est le propriétaire
} else {
    // Vérifier si l'utilisateur a accès via la table resource_professors_access
...
<head>
    ...
    <link rel="canonical" href="http://studtraj.alwaysdata.net/views/user/resource_details.php?id=<?= $resource->resource_id ?>">
    <style>
        /* Styles des TPs (inchangés) */
        .tp-list-container {
            padding: 20px;
            max-width: 900px;
            margin: 20px auto;
...
<body>
    ...
    <div class="main-content">
        <div class="resource-details-header">
            ...
            <div class="owner-info">Créée par <?= htmlspecialchars($resource->owner_firstname . ' ' . $resource->owner_lastname) ?></div>
        </div>
                ...
                <?php foreach ($exercises as $exercise): ?>
                    <div class="tp-item">
                        <div class="tp-item-info">
                            ...
                            <p>Difficulté: <?= htmlspecialchars($exercise->difficulte ?? 'Non spécifiée') ?></p>
    ...
    <script>
        // Fonctions JavaScript (copie de resources_list.js ou intégrées si très simples)
